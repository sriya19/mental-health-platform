"""
Index all non-indexed datasets in the system
"""
import requests
import time
...
def index_dataset(org, uid, dataset_name):
    ...
    try:
        ...
        print(f"⏱️ TIMEOUT: Indexing took longer than 5 minutes")
        return False, 0
...
def main():
    print("="*80)
    print("INDEXING ALL NON-INDEXED DATASETS")
    print("="*80)
    print(f"Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # Get all datasets
    print("\nFetching datasets from backend...")
    ...
    if not non_indexed:
        print("✅ All datasets are already indexed!")
        return
    ...
    print(f"\n{'='*80}")
    confirm = input(f"Index all {len(non_indexed)} datasets? This may take a while. (y/n): ")
    ...
    for i, dataset in enumerate(non_indexed, 1):
        ...
        if not uid or uid == "None":
            print(f"\n[{i}/{len(non_indexed)}] ⚠️ SKIPPING: {name} (No UID)")
            failed += 1
            continue
